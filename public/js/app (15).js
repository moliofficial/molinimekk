...
function _loadPageData(name, rawUrl) {
  ...
  else if (name === 'detail' && rawUrl) loadDetail(rawUrl);
  else if (name === 'watch' && rawUrl) loadWatch(rawUrl, '', currentAnimeUrl, currentAnimeTitle, currentAnimeImage, currentEpisodes);
...
function _routeFromPath(path, search) {
  ...
  else if (path === '/latest') { _activatePage('latest'); loadLatest(1); }
  else if (path === '/genres') { _activatePage('genres'); loadGenres(); }
  else if (path === '/history') { _activatePage('history'); renderHistoryPage(); }
  else if (path === '/search') { _activatePage('search'); }
  else if (path === '/detail') {
    const u = params.get('url');
    if (u) { _activatePage('detail'); loadDetail(u); }
    else { _activatePage('home'); loadHome(); }
  }
  else if (path === '/watch') {
    const u = params.get('url');
    if (u) { _activatePage('watch'); loadWatch(u, '', '', '', '', []); }
    else { _activatePage('home'); loadHome(); }
  }
  else if (path.startsWith('/genre/')) {
    const slug = path.replace('/genre/', '').replace(/\/$/, '');
    if (slug) { _activatePage('genre-detail'); loadGenreDetail(slug, slug); }
    else { _activatePage('genres'); loadGenres(); }
  }
  else { _activatePage('home'); loadHome(); }
}
...
async function loadLatest(page = 1) {
  latestPage = page;
  showPage('latest', true, null, page > 1 ? `/latest?page=${page}` : '/latest');
...
async function loadGenres() {
  showPage('genres', true, null, '/genres');
...
async function loadGenreDetail(slug, name, page = 1) {
  ...
  showPage('genre-detail', true, slug, `/genre/${slug}${page > 1 ? `?page=${page}` : ''}`);
...
async function loadDetail(url) {
  showPage('detail', true, url, `/detail?url=${encodeURIComponent(url)}`);
...
async function loadWatch(url, title, animeUrl, animeTitle, animeImage, episodes) {
  showPage('watch', true, url, `/watch?url=${encodeURIComponent(url)}`);
